...
export default function Dashboard({ onNavigate }) {
  ...
  const sentTotal = sumAda(transactions.filter((tx) => (tx.type || "sent") === "sent"));
  const pendingCount = transactions.filter((tx) => tx.status === "pending" || tx.status === "submitted").length;
  const txCount = transactions.length;
  const recentTransactions = transactions.slice(0, 5);

  const STATS = [
    { label: "Total Sent", value: `${sentTotal} ₳`, sub: "All submitted txs", icon: ArrowUpRight },
    { label: "Pending", value: `${pendingCount}`, sub: "Awaiting confirmation", icon: RefreshCw },
    { label: "Transactions", value: `${txCount}`, sub: "Recorded by backend", icon: TrendingUp },
    { label: "Linked Wallet", value: transactions.length > 0 ? "Active" : "—", sub: "CIP-30 wallet", icon: Wallet },
  ];
  return (
    <div>
